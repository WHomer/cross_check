module TeamStatistics
  ...
  def worst_loss(input)
    data = @combine_data.find_all { |game| game[:team_id] == input.to_i && game[:won] == 'FALSE' }
    result = data.max_by { |game| (game[:away_goals] - game[:home_goals]).abs }
  ...
  def favorite_opponent(input)
    ...
    data = games.each_with_object({}) do |game, hash|
      next unless game[1][:teams][0] == input.to_i || game[1][:teams][1] == input.to_i

      loser_id = game[1][:loser]
      win_id = game[1][:winner]
      hash[loser_id] = { loss: 0, total_games: 0, average: 0 } if hash[loser_id].nil?
      hash[win_id] = { loss: 0, total_games: 0, average: 0 } if hash[win_id].nil?
      if game[1][:winner] == input.to_i
        hash[loser_id][:loss] += 1
        hash[loser_id][:total_games] += 1
        hash[loser_id][:average] = (hash[loser_id][:loss].to_f / hash[loser_id][:total_games])
      else
        hash[win_id][:total_games] += 1
        hash[win_id][:average] = (hash[win_id][:loss].to_f / hash[win_id][:total_games])
      end
    end
    team_id = data.max_by { |team_data| team_data[1][:average] }
  ...
  def head_to_head(team_id)
    ...
    team_names_hash = {}
    all_team_ids_hash.each do |_team|
  ...
  def seasonal_summary(team_id)
    summary = @games.uniq { |game| game[:season] }.each_with_object({}) do |season, hash|
        ...
        if game[:type] == 'R'
          if game[:team_id] == team_id
            if game[:home_team_id] == team_id
              summary[season_id][:regular_season][:total_goals_scored] += game[:home_goals]
            elsif game[:away_team_id] == team_id
              summary[season_id][:regular_season][:total_goals_scored] += game[:away_goals]
            end
            summary[season_id][:regular_season][:number_of_games] += 1
            summary[season_id][:regular_season][:number_of_wins] += 1 if game[:won] == 'TRUE'
          elsif game[:team_id] != team_id
            if game[:home_team_id] != team_id
              summary[season_id][:regular_season][:total_goals_against] += game[:home_goals]
            elsif game[:away_team_id] != team_id
              summary[season_id][:regular_season][:total_goals_against] += game[:away_goals]
            end
          end
          summary[season_id][:regular_season][:average_goals_scored] = (summary[season_id][:regular_season][:total_goals_scored].to_f / summary[season_id][:regular_season][:number_of_games]).round(2)
          summary[season_id][:regular_season][:average_goals_against] = (summary[season_id][:regular_season][:total_goals_against].to_f / summary[season_id][:regular_season][:number_of_games]).round(2)
          summary[season_id][:regular_season][:win_percentage] = (summary[season_id][:regular_season][:number_of_wins].to_f / summary[season_id][:regular_season][:number_of_games]).round(2)
        elsif game[:type] == 'P'
          if game[:team_id] == team_id
            if game[:home_team_id] == team_id
              summary[season_id][:postseason][:total_goals_scored] += game[:home_goals]
            elsif game[:away_team_id] == team_id
              summary[season_id][:postseason][:total_goals_scored] += game[:away_goals]
            end
            summary[season_id][:postseason][:number_of_games] += 1
            summary[season_id][:postseason][:number_of_wins] += 1 if game[:won] == 'TRUE'
          elsif game[:team_id] != team_id
            if game[:home_team_id] != team_id
              summary[season_id][:postseason][:total_goals_against] += game[:home_goals]
            elsif game[:away_team_id] != team_id
              summary[season_id][:postseason][:total_goals_against] += game[:away_goals]
            end
          end
          if summary[season_id][:postseason][:number_of_games] > 0
            summary[season_id][:postseason][:average_goals_scored] = (summary[season_id][:postseason][:total_goals_scored].to_f / summary[season_id][:postseason][:number_of_games]).round(2)
            summary[season_id][:postseason][:average_goals_against] = (summary[season_id][:postseason][:total_goals_against].to_f / summary[season_id][:postseason][:number_of_games]).round(2)
            summary[season_id][:postseason][:win_percentage] = (summary[season_id][:postseason][:number_of_wins].to_f / summary[season_id][:postseason][:number_of_games]).round(2)
          end
        end
      end
    end
    summary.each do |season|
